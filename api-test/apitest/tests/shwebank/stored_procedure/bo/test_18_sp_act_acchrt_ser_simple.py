...
@pytest.mark.SP_ACT_ACCHRT_SER_SIMPLE
class Test_SP_ACT_ACCHRT_SER_SIMPLE(object):

    def test_sp_act_acchrt_ser_simple_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.ACT_ACCHRT_SER_SIMPLE(
            page_size=page_size
        )
        rs = sp_helper.ACT_ACCHRT_SER_SIMPLE(fields_data)
        step_code = 'SQL_SER_SIMPLE_ACCHRT'
