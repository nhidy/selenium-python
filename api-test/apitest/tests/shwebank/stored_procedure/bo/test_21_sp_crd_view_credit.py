...
@pytest.mark.SP_CRD_VIEW_CREDIT
class Test_SP_CRD_VIEW_CREDIT(object):

    def test_sp_crd_view_credit_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.CRD_VIEW_CREDIT(
            id=id
        )
        rs = sp_helper.CRD_VIEW_CREDIT(fields_data)
        step_code = 'SQL_VIEW_CREDIT'
