...
@pytest.mark.SP_PMT_SEARCH_SP_QUEUE_INWAR
class Test_SP_PMT_SEARCH_SP_QUEUE_INWAR(object):

    def test_sp_pmt_search_sp_queue_inwar_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.PMT_SEARCH_SP_QUEUE_INWAR(
            page_size=page_size
        )
        rs = sp_helper.PMT_SEARCH_SP_QUEUE_INWAR(fields_data)
        step_code = 'SQL_SEARCH_SP_QUEUE_INWAR'
