...
@pytest.mark.SP_CRD_SEARCH_SP_CREDIT
class Test_SP_CRD_SEARCH_SP_CREDIT(object):

    def test_sp_crd_search_sp_credit_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.CRD_SEARCH_SP_CREDIT(
            page_size=page_size
        )
        rs = sp_helper.CRD_SEARCH_SP_CREDIT(fields_data)
        step_code = 'SQL_SEARCH_SP_CREDIT'
