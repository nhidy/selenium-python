...
@pytest.mark.SP_ADM_ADVANCED_SEARCH_ROLE_OF_USER
class Test_SP_ADM_ADVANCED_SEARCH_ROLE_OF_USER(object):

    def test_sp_adm_advanced_search_role_of_user_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.ADM_ADVANCED_SEARCH_ROLE_OF_USER(
            role_id=role_id
        )
        rs = sp_helper.ADM_ADVANCED_SEARCH_ROLE_OF_USER(fields_data)
        step_code = 'SQL_ADVANCED_SEARCH_ROLE_OF_USER'
