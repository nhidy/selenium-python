...
@pytest.mark.SP_ADM_REFRESH_BRANCH
class Test_SP_ADM_REFRESH_BRANCH(object):

    def test_sp_adm_refresh_branch_001_success(self, user):
        sp_helper = StoredProcedureHelper(user)
        fields_data = sp_payload.ADM_REFRESH_BRANCH(
            page_index=page_index,
            page_size=page_size
        )
        rs = sp_helper.ADM_REFRESH_BRANCH(fields_data)
