...
account_name_saving_deposit='TEST AUTO SAVING ' + date_time
...
catalog_code_fixed_1m_deposit='FD01POMMK'
catalog_name_fixed_1m_deposit='Fixed deposit 1 months (Principal rollover only) in MMK'
deposit_type_fixed_1m_deposit='Fixed Deposit'
deposit_sub_type_fixed_1m_deposit='T1'
deposit_purpose_fixed_1m_deposit='S'
account_type_fixed_1m_deposit='1'
account_name_fixed_1m_deposit='TEST AUTO FD 1M ' + date_time
rollover_fixed_1m_deposit='P'
auto_transfer_option_fixed_1m_deposit='I'

# data approve deposit
description_dpt_apr='Approve deposit account'

# data cash deposit saving
...
@pytest.mark.open_individual_fd_account
class TestOpenIndividualFDAccount(object):
    ...
    def test_001_open_individual_deposit_account(self, user):
        ...
        while i < 201:
            ...
            global account_number_new_fixed_1m_deposit_001
            account_number_new_fixed_1m_deposit_001 = ''
            fields_data = deposit_fo_payload.dpt_opn(
                customer_type=customer_type,
                customer_code='11056753',
                catalog_code=catalog_code_fixed_1m_deposit,
                catalog_name=catalog_name_fixed_1m_deposit,
                deposit_type=deposit_type_fixed_1m_deposit,
                deposit_sub_type=deposit_sub_type_fixed_1m_deposit,
                deposit_purpose=deposit_purpose_fixed_1m_deposit,
                account_type=account_type_fixed_1m_deposit,
                account_name=account_name_fixed_1m_deposit + ' ' + str(i),
                rollover=rollover_fixed_1m_deposit,
                auto_transfer_option=auto_transfer_option_fixed_1m_deposit,
            ...
            try:
                rs = helper_fo_dpt.DPT_OPN(fields_data)
                assert 'account_number' in rs, f'Key \"account_number\" does not exist. Response Json: {json.dumps(rs, indent=4, sort_keys=True)}'
                account_number_new_fixed_1m_deposit_001 = rs['account_number']
                assert 'transaction_number' in rs, f'Key \"transaction_number\" does not exist. Response Json: {json.dumps(rs, indent=4, sort_keys=True)}'
                transaction_number = rs['transaction_number']
                # assert transaction_number != ''
                print('Transaction number DPT_OPN: ', transaction_number)
            ...
            fields_data = deposit_fo_payload.dpt_apr(
                account_number=account_number_new_fixed_1m_deposit_001,
                account_holder_name=account_name_fixed_1m_deposit,
                description=description_dpt_apr
            )
            try:
                rs = helper_fo_dpt.DPT_APR(fields_data)
            ...
            fields_data = deposit_fo_payload.dpt_cdp(
                ...
                cash_currency=currency_code_mmk,
                account_name=account_name_fixed_1m_deposit,
                exchange_rate=exchange_rate,
                cross_rate=exchange_rate,
