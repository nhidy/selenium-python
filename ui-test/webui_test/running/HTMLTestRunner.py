...
from webui_test.jwebui_action import get_driver
...
class _TestResult(TestResult): 
	...
	def addSuccess(self, test): 
		...
		# try:
		# 	test.imgs.append(get_driver().get_screenshot_as_base64())
	...
	def addError(self, test, err):
		...
		try:
			...
			log.error(msg)
		# if WebUI_Test.driver is not None: 
		# 	try: 
		# 		test.imgs.append(WebUI_Test.driver.get_screenshot_as_base64()) 
		# 	except BaseException as msg: 
		# 		log.error(msg)
		if self.verbosity > 1:
			sys.stderr.write('E  ')
...
class HTMLTestRunner(Template_mixin): 
	def __init__(self, stream=sys.stdout, verbosity=1, title=None, description=None, save_last_run=True, browser=None):
		self.stream = stream 
		self.verbosity = verbosity 
		self.save_last_run = save_last_run
		self.run_times = 0
	...
	def getReportAttributes(self, result): 
		...
		return [
			{"name": "Browser", "value": self.browser},
	...
	def _generate_heading(self, report_attrs):
		self.description = f'{self.description} On {BrowserConfig.url_env}'
